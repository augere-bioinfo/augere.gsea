# library(testthat); library(augere.gsea); source("test-generateGoseqCommands.R")

set.seed(999)

test_that("generateGoseqCommands works as expected", {
    ngenes <- 1000
    sets <- list(
        A=1:20,
        B=50:100,
...
test_that("generateGoseqCommands works with other alternative hypotheses", {
    ngenes <- 1000
    sets <- list(
        A=1:20,
        B=451:470,
...
test_that("generateGoseqCommands works with empty sets", {
    ngenes <- 1000
    sets <- list(
        A=1:10,
        B=integer(0),
